...
if __name__ == "__main__":
    ...
    #iterating over the rows of the dataframe in the reverse order
    #for i, row in tqdm.tqdm(db_df.iloc[::-1].iterrows()):
        #only first 150 videos
        if i > 150:
            break
        cut_and_rescale_video(
                    ...
                    cut_output_filepath=Path(settings['cut_stimulus_video_directory']) / f'{str(row["v_index"]).zfill(4)}_rv.mp4',
                    rescaled_output_filepath=Path(settings['rescaled_stimulus_video_directory']) / f'{str(row["v_index"]).zfill(4)}_rv.mp4')
