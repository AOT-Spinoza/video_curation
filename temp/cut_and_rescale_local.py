...
if __name__ == "__main__":
    ...
    for direction, dir_label in zip([False, True], ['fw', 'rv']):
        Parallel(n_jobs=settings['preferred_n_jobs_transcode'], backend="multiprocessing")(delayed(cut_and_rescale_video)(
                    video_filepath=Path(settings['raw_video_directory_local']) / row['file_name'],
                    tmin=row['tmin'],
                    tmax=row['tmax'],
                    ffmpeg_settings=settings['ffmpeg_settings'],
                    reverse=direction,
                    hflip=bool(row['horizontal_flip']),
                    cut_output_filepath=Path(settings['cut_stimulus_video_directory_local']) / f'{str(row["v_index"]).zfill(4)}_{dir_label}.mp4',
                    rescaled_output_filepath=Path(settings['rescaled_stimulus_video_directory_local']) / f'{str(row["v_index"]).zfill(4)}_{dir_label}.mp4')
